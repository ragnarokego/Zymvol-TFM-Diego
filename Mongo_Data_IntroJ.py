...
def introduce_data(list_tags): #Function to give the order to introduce the information for the specified properties in the list of tags. Not recommended for the whole ligand information. 
	...
	for tag in list_tags:
		for num in ["3.5.5.2", "3.5.5.5", "3.5.5.6", "3.5.5.7", "3.5.5.8"]:
			counter = 0
			for line in open("Property_Info2/%s/%s.txt" % ( tag, num), "rt"):
				...
				counter += 1
			print("Output_%s_%s.txt entries were introduced in the database." %(tag, num))
...
def introduce_ligands(lig_entries, start, stop): #Function to give the order to introduce the information for ligands.
	...
	for num in ["3.5.5.2", "3.5.5.5", "3.5.5.6", "3.5.5.7", "3.5.5.8"]:
		...
		print(num)
		for line in open("Property_Info2/lig/%s.txt" % (num), "rt"):
		...
		print("Output_lig_%s.txt entries were introduced in the database." %(num))
	print("Tag lig entries were introduced in the database." )

def relate_accession_ic50(content_dict):
	
	#We introduce the sequence related to the experiment
	accessionfile = open("Protseq_Relation_2/%s.txt" % (content_dict["ecNumber"]) , "rt") #We open the file with the data that relates the kinetic info to the accession code of the sequence
	...
	for line in accessionfile:
		...
		if content_dict["commentary"] == info[3] and content_dict["literature"][0] == int(info[1]) and content_dict["ic50Value"] == float(info[2]): #The identifiers are recognized
		...
		elif content_dict["commentary"] != info[3] or content_dict["literature"][0] != int(info[1]) or str(content_dict["icValue"]) != info[2]: #No identifiers recognized or not all means that there is no sequence associated
...
list_tags =['seq', 'km', 'kcatkm', 'turnnum', 'lig', 'recname']
